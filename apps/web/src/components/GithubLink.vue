<template>
  <a class="ghlink" :style="{ width: size + 'px', height: size + 'px' }" :href="REPO" target="_blank" rel="noopener noreferrer" title="Source on GitHub" aria-label="View the source on GitHub">
    <svg viewBox="0 0 24 24" :width="Math.round(size * 0.56)" :height="Math.round(size * 0.56)" aria-hidden="true">
      <path fill="currentColor" d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.51 11.51 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222 0 1.606-.014 2.898-.014 3.293 0 .322.216.694.825.576C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/>
    </svg>
  </a>
</template>

<script setup>
defineProps({ size: { type: Number, default: 40 } });
const REPO = 'https://github.com/virgilvox/doot-doot-revolution';
</script>

<style scoped>
.ghlink { display: inline-grid; place-items: center; color: var(--ink); border-radius: 999px; text-decoration: none; transition: transform .1s var(--sp), background .12s; }
.ghlink svg { display: block; }
.ghlink:hover { transform: translateY(-1px); background: rgba(34, 32, 63, .1); }
.ghlink:active { transform: translateY(0); }
</style>
