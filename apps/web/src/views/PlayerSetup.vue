...
const order = computed(() => [
  ...
  { device: 'keyboard', label: 'Keyboard (arrows)' },
  { device: 'keyboard2', label: 'Keyboard (WASD)' }
].slice(0, 4));
...
const deviceLabel = (i) => (order.value[i] ? order.value[i].label : '—');
function inc() { if (count.value < maxPlayers.value) count.value++; }
function dec() { if (count.value > 1) count.value--; }

let aborted = false;
async function start() {
  if (busy.value || !song) return; busy.value = true;
  try {
    const n = count.value;
    if (n === 1) {
